...
class Solution
{
   public:
    ...
    bool check(Node* root, int k, int dist) {
        if (root == nullptr)
            return 0;
        if (dist == k and root->left == nullptr and root->right == nullptr)
            return 1;
        else if (root->left == nullptr and root->right == nullptr and dist != k)
            return 0;
        
        return check(root->left, k, dist + 1) or check(root->right, k, dist + 1);
    }
